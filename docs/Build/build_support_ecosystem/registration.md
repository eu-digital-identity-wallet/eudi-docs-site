...
To integrate with the [RI Verifier](../build-your-verifier/index.md) in order to load from a keystore the key used for JAR-signing authorisation requests ([variable `VERIFIER_JAR_SIGNING_KEY_KEYSTORE`](../build-your-verifier/configure.md)), please use the following command line to convert the downloaded PKCS#12 file to a JKS file:
