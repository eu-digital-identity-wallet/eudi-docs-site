...
| Performance | - _Android Studio Profiler for local device execution on real Android device, - _Xcode Instruments for local device execution on real iOS device, - _BrowserStack App Live for tests executed across multiple real Android and iOS devices in the BrowserStack device farm.|
