...
| Performance | - _Android Studio Profiler_ for local device execution on real Android device, - _Xcode Instruments_ for local device execution on real iOS device, - _BrowserStack App Live_ for tests executed across multiple real Android and iOS devices in the BrowserStack device farm.|
